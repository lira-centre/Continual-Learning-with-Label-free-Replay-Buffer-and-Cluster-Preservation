...
def contrastive_push_away_loss(new_features, prototypes_input, prototypes_latent, previous_means, previous_stds, model, epsilon=1e-6):
    ...
    for cls in prototypes_input:
        ...
        #prototype_latent = model(prototype_input).T
        ...
        #scaled_sim = (sim)/((1 - sigma) * (1/temperature))  # Shape: (batch_size,)
        scaled_sim = (sim)/((1 - sigma) *7)
...
def main():
    ...
    for task_id in range(num_tasks):
        ...
        # Calculate and print Backward Transfer (BWT)
        if task_id > 0:
            bwt = 0
            for prev_task_id in range(task_id):
                bwt += task_accuracies[prev_task_id][-1] - task_accuracies[prev_task_id][0]  # Final vs Initial accuracy
            bwt /= task_id
            print(f"Backward Transfer after Task {task_id + 1}: {bwt:.2f}%")
