...
def select_prototypes(features, features_transformed, labels, cluster_labels, class_means, prototype_labels, task_id, num_classes, model):
    ...
    for index, cluster_center in class_means.items():
        ...
        #prototypes_input[prototype_label.item()] = proto_input
        ...
        #prototypes_latent[prototype_label.item()] = proto_latent
...
def contrastive_pull_to_loss(new_features, labels, prototypes_input, prototypes_latent, previous_means, previous_stds, model, num_classes, task_id):
    device = new_features.device
    if not previous_means:
        # No previous classes, return zero loss
        return torch.tensor(0.0, device=device)
    ...
    # Prepare list to collect mean losses per cluster
    mean_losses = []
    
    for cls in prototypes_input:
        # Only first task prototypes
        if cls < num_classes:
            prototype_input = prototypes_input[cls].unsqueeze(0).to(device)  # Shape: (1, feature_dim)
            # Compute new representation of the prototype in latent space
            prototype_latent = prototypes_latent[cls].unsqueeze(0).T.to(device)
            ...
            class_mask = (labels == cls + (task_id * num_classes))
            class_features = new_features[class_mask]

            if class_features.shape[0] > 0:  # Check if there are features for this class
                # Compute cosine similarity between new features and prototype
                sim = torch.matmul(class_features, prototype_latent)  # Shape: (num_samples,)
                
                # Scale similarity by sigma-adjusted temperature
                scaled_sim = 1 - sim

                # Directly minimize the scaled similarity
                loss = scaled_sim.mean()  # Compute mean for this class
                
                # Accumulate mean loss
                mean_losses.append(loss)
    
    if mean_losses:
        # Compute overall mean loss across clusters
        total_loss = torch.mean(torch.stack(mean_losses))
    else:
        # If no losses were accumulated, return zero loss
...
def nearest_prototype_classification(features, prototypes, prototype_labels):
    ...
    """
    predictions = []
    prototype_labels = [x if x <= classes_per_task - 1 else x % classes_per_task for x in prototype_labels]
...
def main():
    ...
    # Store accuracy for each task at each stage
    ...
    for task_id in range(num_tasks):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_features, batch_labels in train_loader:
                ...
                task_loss = contrastive_loss_fn(transformed_features, batch_labels)
                ...
                if previous_classes:
                    ...
                    pull_loss = contrastive_pull_to_loss(transformed_features, batch_labels, prototypes_input, prototypes_latent, prev_means, prev_stds, model, classes_per_task, task_id)
                else:
                    pull_loss = 0.0
                
                
                # Total loss
                total_loss = task_loss + lambda_preserve * preserve_loss + lambda_pull * pull_loss
        ...
        for eval_task_id in range(task_id + 1):
            ...
            all_test_labels = torch.where(all_test_labels <= classes_per_task - 1, all_test_labels, all_test_labels % classes_per_task)
            ...
            total_seen_tasks_accuracy.append(accuracy * 100)

            print(f"Test Accuracy on Task {eval_task_id + 1}: {accuracy * 100:.2f}%")

        # Calculate overall accuracy on all seen tasks so far
        overall_accuracy = sum(total_seen_tasks_accuracy) / len(total_seen_tasks_accuracy)
        print(f"Overall Accuracy on All Seen Tasks after Task {task_id + 1}: {overall_accuracy:.2f}%")
