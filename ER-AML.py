...
# Adjusted Buffer fetch_pos_neg_samples to ensure task is a tensor
class Buffer(nn.Module):
    ...
    def fetch_pos_neg_samples(self, label, task, idx, data=None, task_free=True):
        ...
        # Expand buffer task and label tensors for comparison
        ...
        # Handle edge cases where no valid samples exist
...
def main():
    ...
    for task_id in range(num_tasks):
        ...
        # Optionally, print the average accuracy over tasks 1..(task_id+1)
        ...
        # Compute BWT
        # --------------
        # We'll compute BWT if task_id > 0, because we need at least one previous task.
        if task_id > 0:
            bwt_sum = 0.0
            for prev_task_id in range(task_id):
                acc_after_current = task_accuracies[prev_task_id][-1]
                acc_after_own_training = task_accuracies[prev_task_id][0]
                bwt_sum += (acc_after_current - acc_after_own_training)
            bwt = bwt_sum / task_id
            print(f"Backward Transfer (BWT) after Task {task_id + 1}: {bwt:.2f}%")
